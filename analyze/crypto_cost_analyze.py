...
def get_crypto_users(platform):
    config = configparser.ConfigParser()
    config.read('config.ini')
    data = config['users']
    owners = [x.strip() for x in data[platform+'_users'].split(',')]
    print(owners)
    return owners
...
def parse_binance_trades(file):
    ...
    fee = df.groupby('Fee Coin').agg("sum")
...
def parse_binance_p2p(file, process_trades):
    ...
    if process_trades:
        ...
        if len(tfiles)>0:
            ...
            mkt = pd.concat([mkt, newrow], ignore_index=False)
            mkt = mkt.drop('BNBINR')
...
for user in get_crypto_users('coindcx'):
    ...
    for file in files:
        ...
        df[user] = pd.concat([df[user], dftemp], ignore_index=False)
    tdf = pd.concat([tdf, df[user]], ignore_index=False)

for user in get_crypto_users('binance'):
...
for user in get_crypto_users('wazirx'):
...
print('\n**** Aggregate ****\n')
tdf = tdf.drop(columns=['Owner', 'Held in', 'Desc'])
tdf = tdf.groupby('Symbol').agg("sum")
